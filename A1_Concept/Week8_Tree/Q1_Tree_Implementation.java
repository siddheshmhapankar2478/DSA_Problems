...
public class Q1_Tree_Implementation {
            ...
            while(!q.isEmpty()){
                ...
                for(int i=0;i<levelSize;i++){
                    if (leftToRight) 
                        ...
                        current.add(0, q.peek().val); // Insert at the beginning for right-to-left
    ...
    class Solution20 {
        ...
        private TreeNode buildTree(int[] preorder, int preStart, int preEnd, int[] postorder, int postStart, int postEnd) {
            ...
            root.right = buildTree(preorder, preStart + leftSubtreeSize + 1, preEnd, postorder, postIdx + 1, postEnd - 1);
